...
class TestJsonParser(unittest.TestCase):
    ...
    def test_001_file_pattern_accetance(self) -> None:
        self.assertTrue(JsonFileParser.accepts("/no/path/file.json"), ".json extensions is supported")
        self.assertFalse(JsonFileParser.accepts("/no/path/file"), "files without extensions are not supported")
        self.assertFalse(JsonFileParser.accepts("/no/path/file.txt"), "only json extension is supported")
